...
export class SocketManager {
    ...
    constructor() {
        clientEventsEmitter.registerToClientJoin((clientUUid: string, roomId: string, totalUsersCount: number) => {
            gaugeManager.incNbClientPerRoomGauge(roomId);
            gaugeManager.currentSocketsConnectedGauge(totalUsersCount);
        });
        clientEventsEmitter.registerToClientLeave((clientUUid: string, roomId: string, totalUsersCount: number) => {
    ...
    leaveRoom(room: GameRoom, user: User){
        ...
        try {
            ...
            clientEventsEmitter.emitClientLeave(user.uuid, room.roomId, this.getTotalUsersCount());
            console.log(new Date().toISOString() + ` User "${user.name}" (${user.IPAddress}) left`);
        }
    }
    ...
    private async joinRoom(socket: UserSocket, joinRoomMessage: JoinRoomMessage): Promise<{ room: GameRoom; user: User }> {
        ...
        clientEventsEmitter.emitClientJoin(user.uuid, roomId, this.getTotalUsersCount());
        //console.log(new Date().toISOString() + ' A user joined (', this.sockets.size, ' connected users)');
        console.log(new Date().toISOString() + ` User "${user.name}" (${user.IPAddress}) joined`);
    ...
    public getTotalUsersCount(): number {
        return Array.from(this.rooms.values()).reduce((prevValue, gameRoom) => prevValue + gameRoom.getUsers().size, 0);
    }
}
